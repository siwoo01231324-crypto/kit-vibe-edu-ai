...
```
1. 작업 완료 후 docs/ai-report/daily-log.md에 AI 활용 내역 업데이트 필수
   — 어떤 AI 도구로, 어떤 작업을, 어떻게 수행했는지 기록
   — 생략 시 작업 미완료로 간주
2. AI가 생성/수정한 코드는 반드시 인간 검토 후 커밋
3. docs/ai-report/ 하위 문서에는 팩트 기반 내용만 작성 (허구 금지)
```
...
5. **완료 후 `docs/ai-report/daily-log.md`에 AI 활용 내역 기록 — 필수 (불변식 1번)**
   - 사용 도구, 주요 프롬프트, AI 기여 영역, 인간 주도 영역을 해당 일자 섹션에 추가
6. 실패 시 → "레포에 무엇이 없었나?" 진단 → 문서 업데이트
